...
!---------------------------------------------------------------
! Subroutine: 将非正交基底下的 H 和 S 转换到正交基底
!   输入  Hamk_bulk(N,N): 待正交化的 Hamiltonian
!        Sk_bulk(N,N) : 重叠矩阵 S
!   输出  Hamk_bulk    : 已正交化的新 Hamiltonian
!---------------------------------------------------------------
subroutine orthogonalize_hamiltonian(Hamk_bulk, Sk_bulk , N)
   ...
   ! 1) 对 S 做特征分解： S_vec * diag(S_vals) * S_vec^H = Sk_bulk
   S_vec = Sk_bulk
   call eigensystem_c('V','U', N, S_vec, S_vals)
 
   ! 2) 构造 M_inv = diag(1/√S_vals)
   M_inv = (0.0_dp,0.0_dp)
   ! do i = 1, N
   ...
   ! do i=1, N
   !    do j=1, N
   !       ! if (abs(Hamk_bulk(i,j)-conjg(Hamk_bulk(j,i)))>1.0e-10_dp) then
   !       if (abs(Sk_bulk(i,j)-conjg(Sk_bulk(j,i)))>1e-10) then
   !       ! if (abs(U(i,j)-conjg(U(j,i)))>1.0e-10_dp) then
   !          write(*, *) 'Warning: The test Hamiltonian is not Hermitian!'
   !          write(*, *) 'i,j, Hamk_bulk(i,j), Hamk_bulk(j,i)', i, j, &
   !             Hamk_bulk(i,j), Hamk_bulk(j,i)
   !          exit
   !       endif
   !    enddo
   ! enddo
 
   ! 4) 新的 Hamiltonian = U * Hamk_bulk * U
   ! Hamk_bulk = matmul( U, matmul(Hamk_bulk, U) )
   Hamk_bulk = matmul(U, Hamk_bulk)
   Hamk_bulk = matmul(Hamk_bulk, U)
   !check if the new Hamiltonian is Hermitian
   ! if (any( abs(Hamk_bulk - conjg(transpose(Hamk_bulk))) > 1.0e-10_dp )) then
   !    write(*, *) 'Warning: The orthogonalized Hamiltonian is not Hermitian!'
   !    write(*, *) 'Please check the input S matrix or the orthogonalization process.'
   ! end if
   ! do i=1, N
   !    do j=1, N
   !       ! if (abs(Hamk_bulk(i,j)-conjg(Hamk_bulk(j,i)))>1.0e-10_dp) then
   !       if (abs(U(i,j)-conjg(U(j,i)))>1e-10) then
   !          write(*, *) 'Warning: The orthogonalized Hamiltonian is not Hermitian!'
   !          write(*, *) 'i,j, Hamk_bulk(i,j), Hamk_bulk(j,i)', i, j, &
   !             Hamk_bulk(i,j), Hamk_bulk(j,i)
   !          exit
