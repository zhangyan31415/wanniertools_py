...
!-----------------------------------------------------------------------------------------------------!
module me_calculate
    use para, only: dp, pi, zi, photon_energy_arpes, polarization_phi_arpes, &
                    polarization_xi_arpes, polarization_alpha_arpes, polarization_delta_arpes, &
                    penetration_lambda_arpes
    use element_table, only: lines_of_table, angular_number, a_0, lenth_of_table, magnetic_number_max, &
                             get_element_index, get_orbital_index, get_electron_config, get_valence_config, &
                             factorial, orb_sign, element_name
...
!> count the number of zero in input m values, we will use it in the following subroutine       
    ...
    subroutine gaunt_real_basis(l, m, dl, dm, realbasis_gaunt_value , k_theta, k_phi)
        ...
        if ( (m+dm)<0 ) then
            ...
            sign_gaunt_real = (-1)**(m+dm)*(-1)**(m*m_sign)*(-1)**(dm*dm_sign) + &
                             (-1)**(m+dm)*(-1)**((m-1)*(m_sign-1))*(-1)**((dm-1)*(dm_sign-1))
       end if
       
        realbasis_gaunt_value = realbasis_gaunt_value + i_sign*sign_gaunt_real*1d0/ &
                               (sqrt(2d0))**sum_abs_m*t_gaunt_value* &
                               Ylm(k_theta, k_phi, l+dl, real_or_complex*abs(m+dm))
        ...
        if ( (m-dm)<0 ) then
            ...
            sign_gaunt_real = (-1)**(m-dm)*(-1)**(m*m_sign)*(-1)**((dm-1)*(dm_sign-1)) + &
                             (-1)**(m+dm)*(-1)**((m-1)*(m_sign-1))*(-1)**(dm*dm_sign)
       end if
    ...
    subroutine polarization_vector(polar_epsilon)
        ...
        polar_epsilon(1) = cos(polarization_xi_arpes)*cos(polarization_phi_arpes) - &
                           (cos(polarization_delta_arpes) + zi*sin(polarization_delta_arpes))* &
                           sin(polarization_xi_arpes)*cos(polarization_alpha_arpes)*sin(polarization_phi_arpes)
     
        polar_epsilon(2) = cos(polarization_xi_arpes)*sin(polarization_phi_arpes) + &
                           (cos(polarization_delta_arpes) + zi*sin(polarization_delta_arpes))* &
                           sin(polarization_xi_arpes)*cos(polarization_alpha_arpes)*cos(polarization_phi_arpes)
        
        polar_epsilon(3) = (cos(polarization_delta_arpes) + zi*sin(polarization_delta_arpes))* &
                           sin(polarization_xi_arpes)*sin(polarization_alpha_arpes)
